  ...
  <div>
    ...
    <el-dialog :title="title" :visible.sync="editVisible" width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" v-if="title=='修改学生信息'" @click="saveEdit">确 定</el-button>
        <el-button type="primary" v-if="title=='添加学生信息'" @click="saveAdd">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    saveEdit() {
      ...
      this.editVisible = false;
      this.$axios
        .post("/sysadmin/user/addStudent", this.form)
        .then(res => {
          console.log(this.form);
          if (res.code === 0) {
            this.$message.success(res.msg);
    ...
    getList() {
      ...
      let list2 = this.data.filter((item, index) =>
        item.username.includes(this.keywords)
      );
